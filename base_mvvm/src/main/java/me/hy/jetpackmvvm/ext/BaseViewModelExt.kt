...
import me.hy.jetpackmvvm.ext.util.loge
...
import me.hy.jetpackmvvm.state.ResultState
import me.hy.jetpackmvvm.state.paresException
import me.hy.jetpackmvvm.state.paresResult
 ...
 * @param block 请求体方法
 * @param resultState 请求回调的ResultState数据
 * @param success 额外的成功回调，便于在校验结果后再多一个地方处理业务逻辑
 * @param error 额外的失败回调，便于在校验结果后再多一个地方处理业务逻辑
 * @param isShowDialog 是否显示加载框
 * @param loadingMessage 加载框提示内容
 */
fun <T> BaseViewModel.request(
    ...
    return viewModelScope.launch {
        runCatching {
            ...
            resultState.paresResult(it,success)
        }.onFailure {
            it.message?.loge()
